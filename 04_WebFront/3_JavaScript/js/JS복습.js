...
createBtn.addEventListener("click", e => {
  ...
  while(set.size < bingoLength * bingoLength){
    const r = Math.floor( Math.random() * bingoLength * bingoLength + 1);  // floor 난수생성 소수점 없애기
    set.add(r);
  }
...
function rowCheck(board){
  ...
  for(let i = 0 ; i<board.length ; i++){
    ...
    // 모든 열 접근용 for문 (k인 이유 : 아무 이유없음 그냥함)
    for(let k = 0 ; k < board[i].length ; k++){
...
function diaCheck(board){
  ...
  for(let i = 0 ; i<bingoLength ; i++){
    ...
    // 좌상 -> 우하 요소에서 check 클래스가 있다면
    if(board[i][i].classList.contains("check")){
      count1++;
    }
     // 우상 -> 좌하 요소에서 check 클래스가 있다면
