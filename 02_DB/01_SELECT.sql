...
-- "" 의미
-- 1) 대/소문자 구문
-- 2) 특수문자[영어가 아닌 문자(한글/한문/아랍어 등등)], 띄어쓰기 인식
...
-- DISTINCT (별개의, 따로따로)
--> 조회 시 지정된 컬럼에 중복되는 값을 한 번만 표시할 떄 사용
--> 중복 제거
SELECT DISTINCT DEPT_CODE FROM EMPLOYEE;

--------------------------------------------------------------------------------------------

-- ctrl + shift + 방향키 << 줄 이동 단축키
...
-- 사번, 이름, 부서코드, 전화번호 조회
SELECT EMP_ID AS 사번, EMP_NAME 이름, DEPT_CODE AS "부서코드", PHONE "전화번호"
FROM EMPLOYEE
WHERE DEPT_CODE != 'D9';
...
SELECT EMP_NAME "이름", DEPT_CODE AS "부서코드", SALARY AS 급여
FROM EMPLOYEE
WHERE DEPT_CODE = 'D6' AND SALARY >= 3000000;

-- EMPLOYEE 테이블에서 급여가 300만 이상 500만 이하인 사원의
-- 사번, 이름, 입사일, 급여조회
SELECT EMP_ID 사번, EMP_NAME 이름, HIRE_DATE AS 입사일, SALARY "급여"
FROM EMPLOYEE
...
-- 사번, 이름, 입사일, 급여조회
SELECT EMP_ID AS "사번", EMP_NAME 이름, HIRE_DATE "입사일", SALARY AS 급여
FROM EMPLOYEE
...
-- 이름, 부서코드, 급여 조회
SELECT EMP_NAME 이름, DEPT_CODE "부서코드", SALARY AS "급여"
FROM EMPLOYEE 
...
-- !!!! NULL은 비교, 논리 연산 시 연산의 대상이 되지 않음 !!!!   [ 중 요 ]
...
--> 해결 방법 : LIKE의 ESCAPE 옵션을 사용                    (많이 사용함)
