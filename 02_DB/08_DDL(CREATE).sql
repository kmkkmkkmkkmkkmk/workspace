...
INSERT INTO MEMBER 
VALUES('MEM01', '123ABC', '홍길동', '000854-1154554', DEFAULT);
													-- TO_DATE('2023-08-27')
													-- SYSDATE	
													-- DEFAULT (= SYSDATE)
...
/*
    ...
    제약조건 종류 (필 수 암 기)
    PRIMARY KEY(기본 키, 행을 구분하는 값(사번, 학번) ), 
...
CREATE TABLE USER_USED_UK(
    ...
    --USER_ID VARCHAR2(20) CONSTRAINT USER_IN_U UNIQUE,
      					-- CONSTRAINT	제약조건명 제약조건종류
   					    -- 컬럼 레벨 제약 조건(제약 조건 이름 지정 O)
    USER_ID VARCHAR2(20), -- 컬럼 레벨 제약 조건X
    USER_PWD VARCHAR2(30) ,
    USER_NAME VARCHAR2(30),
    GENDER VARCHAR2(10),
    PHONE VARCHAR2(30),
...
-- ORA-00001: 무결성 제약 조건(A230724_KMK.SYS_C0013073)에 위배됩니다
-- SYS_C0013063 : 제약 조건 이름 미설정 시 자동으로 지정되는 이름
...
-- UNIQUE 복합키 (외워두자 나중에 프로젝트 할 때 많이 쓴다하심****************)
...
CREATE TABLE USER_USED_PK(
	-- 컬럼 레벨로 PK 지정
    -- USER_NO NUMBER PRIMARY KEY, -- 컬럼 레벨, 이름 지정X
    
    -- USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨, 이름 지정O
    ...
    , CONSTRAINT USER_NO_PK PRIMARY KEY(USER_NO) -- 테이블 레벨, 이름 지정O
);
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(A230724_KMK.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("A230724_KMK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(A230724_KMK.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("A230724_KMK"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
----------------------------------------------------------------------------------------------------------------
...
--> 50이라는 값은 USER_GRADE 테이블 GRADE_CODE 컬럼에서 제공하는 값이 아니므로
 ...
 -- ORA-02291: 무결성 제약조건(A230724_KMK.GRADE_CODE_FK1)이 위배되었습니다
--             - 부모 키가 없습니다  
...
/* JOIN : 두 테이블에서 값이 같은 값을 가지고 있는 컬럼을 기준으로 연결
...
DELETE FROM USER_GRADE 
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(A230724_KMK.GRADE_CODE_FK1)이 위배되었습니다
--          - 자식 레코드가 발견되었습니다
...
SELECT * FROM USER_USED_FK3; --홍길동, 이순신 삭제(10을 참조하던 행이 모두 삭제)
...
CREATE TABLE  USER_TEST(
	...
	USER_PWD VARCHAR(100) CONSTRAINT NN_USER_PWD NOT NULL, -- 비밀번호는 넉넉한 게 좋음
...
ALTER TABLE EMPLOYEE ADD -- 무언가를 추가해서 EMPLOYEE 테이블을 변경할꺼다
CONSTRAINT SAL_GRADE_FK -- 제약조건 이름 지정
FOREIGN KEY(SAL_LEVEL) REFERENCES SAL_GRADE;
-- FK제약조건 추가(EMPLOYEE.SAL_LEVEL 컬럼이 SAL_GRADE의 PK 참조)
