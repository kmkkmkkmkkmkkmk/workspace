...
-- SUBSTR(문자열 | 컬럼명, 시작위치 [,길이])                           (실무에서 많이 씀)
...
-- TRIM([ [옵션] 문자열 | 컬럼명 FROM ] 문자열 | 컬럼명)                       (무조건 외워! 실무에서 개 많이씀)
...
-- 소수점 위치 지정 X : 소수점 첫쨰 자리에서 반올림 -> 정수 표현
-- 소수점 위치 지정 O
-- 1) 양수  :  지정된 위치의 소수점 자리까지 표현
-- 2) 음수 :  지정된 위치 정수 자리까지 표현
...
-- MONTH_BETWEEN( ) : 윤년 계산이 알아서 포함되어 있음
...
SELECT SUM(SALARY) 합계, AVG(SALARY) "평균", 
	MAX(SALARY) AS "최대값", MIN(SALARY) AS 최소값
FROM EMPLOYEE;
...
-- COUNT(*) : 조회된 모든 행의 개수를 반환                             (*진짜 많이 씀*)

-- COUNT(컬럼명) : 지정된 컬럼 값이 NULL이 아닌 행의 개수를 반환       (*진짜 많이 씀*)
--				   (NULL인 행 미포함)
