...
-------------------------- 연습문제 -------------------------------
-- 1.  사원과 같은 부서, 같은 직급인 사원을 조회하시오. (단,  사원은 제외)
...
FROM (SELECT EMP_NAME, SALARY
	  FROM EMPLOYEE 
	  ORDER BY SALARY DESC)
WHERE ROWNUM <= 5; --ROWNUM이 1,2,3,4,5 인 행만 조회
...
--WHERE ROWNUM = 2;  -- 2행만 조회 --> 실패!   1행부터 순차적으로 한 게 아니라서 안나옴
WHERE ROWNUM BETWEEN 2 AND 10; --> 1부터 포함해야된다!
 ...
 * 	  EX) 인라인뷰에서 지정된 별칭을 메인 쿼리에서도 똑같이 사용해야된다
 * 
 * 3. 여러 테이블을 이용한 SELECT 진행시
...
-- 3.  사원과 같은 부서, 같은 직급인 사;원을 조회하시오. (단, 노옹철 사원은 제외)
-- 사번, 이름, 부서코드, 직급코드, 부서명, 직급명
