...
public class AjaxController {
	...
	@ResponseBody // 비동기 통신 응답(forward/redirect X, 데이터 자체임)
	public List<Member> selectAll() {
		
		// List (Java 객체)
		// -> HttpMessageConverter가 JSON(문자열)으로 변환
		//	 (+ produces 속성을 이용해 
		//	   응답 받는 JS에서 자동으로 JS객체로 변환하도록함)
		// -> JS 객체
	...
	// @RequestBody (요청 body)
	// - 요청 body에 담긴 내용을 얻어와 오른쪾 매개변수에 대입
	// - HttpMessageConverter가 
	//   이 과정에서 데이터 타입을 Java에 맞게  알맞게 변환
	//   number -> int/double 형태로 변환
	//   string -> String 형태로 변환
	//   JSON -> DTO, List, Map 형태로 변환
	...
	@ResponseBody // 반환 값이 그대로 돌아감(비동기)
