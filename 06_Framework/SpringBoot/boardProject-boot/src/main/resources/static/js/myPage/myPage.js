...
/* 프로필 이미지 미리보기, 제거 */ /* const는 상수라 값을 못바꿈 // let은 값을 바꿀 수 있음 */
const profileImg = document.querySelector("#profileImg"); // img태그
...
if(imageInput != null) { // #imageInput 존재할 때
    ...
    profileFrm.addEventListener("submit", e => {
        ...
        if(flag) { // flag가 true인 경우 수행
            e.preventDefault(); // form 태그 제출 이벤트 막기(제거) == (기본 이벤트 막기)
