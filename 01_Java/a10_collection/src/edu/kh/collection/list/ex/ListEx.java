...
//		List list = new ArrayList(); // 10칸짜리 생성
		List list = new ArrayList(3); // 3칸짜리		
		...
		for(int i = 0 ; i < list.size() ; i++) {
			...
			if(list.get(i) instanceof Integer) { // integer 인 경우
			...
			if(list.get(i) instanceof Double) { // Double 인 경우
				
				Double d = (Double)list.get(i); // 다운 캐스팅만
