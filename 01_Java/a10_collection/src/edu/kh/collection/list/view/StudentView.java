...
public class StudentView {
	...
	/**
	 * 메뉴 출력용 view 메서드   /// (개발할 때 꼭 적어야하는 것!!!!!)
	 */
	public void displayMenu() {
		...
		} while(input !=0); // input이 0이면 flase가 되면서 종료
	}
	...
	private void deleteStudent () {
		...
		System.out.println("\n----- 학생 1명 삭제(index) -----\n");
		System.out.print("삭제할 학생 인덱스 번호 : ");
		...
		if(std == null) {
			...
			System.out.println(std.getName() + "해당 학생을 삭제했습니다.");
		}
	}
...
//		if(searchList.isEmpty())  // 위아래 같은 말임
	...
	private void selectGrade() {
		...
		List<Student> selectList = service.selectGrade(grade);
		if (selectList.size() == 0) {
			System.out.println("검색 결과가 없습니다.");
		} else {
			for(Student s : selectList) {
				System.out.println(s);
			}
		}
	}
	...
	private void duplicateValidation() {
		...
		if(result) {
			...
			System.out.println("같은 학생 없음XXXXXXXX");
		}
	}
