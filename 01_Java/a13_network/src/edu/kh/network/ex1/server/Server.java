...
public class Server {
	...
	public void start() {
		...
		// 1. 서버의 포트 번호 정하기
		int port = 8500;                  // 중요!!!!!!!!!!!!!!! //
		// 포트번호는 0~65535 사이로 지정 가능(2byte 범위 내)
		...
		try { 
			...
			is = clientSocket.getInputStream(); // 클라이언트 -> 서버 입력                              // 중요!!!!!!!!!!!!!!! //
			os = clientSocket.getOutputStream(); // 서버 -> 클라이언트출력                              // 중요!!!!!!!!!!!!!!! //                        
			...
			try { 
				...
				if(br != null) br.close(); //	+ is.close();  << 이건 위 설명처럼 같이 close 되서 안써도됨(써도되긴함)
				if(pw != null) pw.close(); //	+ os.close();  << 위와 동일
