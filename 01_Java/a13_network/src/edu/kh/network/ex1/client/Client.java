...
public class Client {
	...
	public void start() {
		...
		String ip = "127.0.0.1"; // loop back ip(현재 컴퓨터를 나타내는 IP [전세계 약속] )
		...
		try {
			...
			if(socket != null) { // 서버와 연결이 성공한 경우
				...
				// 4-1) 서버 -> 클라이언트로 전달된 메세지를 화면에 출력
				String serverMessage = br.readLine();
				System.out.println(serverMessage);
				
				
				// 4-2) 클라이언트 -> 서버로 입력 받은 메세지 전달
				...
				pw.println(clientMessage); // 서버로 메세지를 출력
				pw.flush(); // 스트림에 있는 메세지 밀어내기
